...
const spectralConfig = path.resolve(__dirname, '../spectral.yaml');

async function runSpectral(file) {
...
describe('Spectral custom rules', () => {
  ...
  test('info-title negative fails', async () => {
    ...
    expect(results.some(r => r.code === 'info-title-not-empty')).toBe(true);
  });
  ...
  test('operation-description negative fails', async () => {
    ...
    expect(results.some(r => r.code === 'operation-description-required')).toBe(true);
  });
});
